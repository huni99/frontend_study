...
// 해당 요소에 지정한 이벤드(Event)가 발생하는지 듣고(Listen) 있다가 
// 실제 이벤트가 발생하면 연결된 함수(Handler)를 실행 
boxEl.addEventListener('click', 
  function(){ 
  console.log('click');  
  //classList 속성: HTML요소의 class속성에 대한 제어이 명령 포함
  // 추가하기
  this.classList.add('active');//요소에 active라는 클래스를 추가

  //확인하기
  let hasActive = this.classList.contains('active');//요소에 active라는 클래스 값이 있는지 확인
  console.log(hasActive);//있으면 true 없으면 false

  // 제거하기
  this.classList.remove('active');//요소에 active라는 클래스가 있으면 제거
  hasActive = this.classList.contains('active');
...
// boxEl.classList.toggle('on'); 위의 코드와 같은 기능을함
});
